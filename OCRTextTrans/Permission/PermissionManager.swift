...
struct PermissionManager {
     ...
     */
    func getCameraAuthorizationStatus() async -> CameraPermission {
        switch AVCaptureDevice.authorizationStatus(for: .video) {
        case .authorized: // 사용자가 카메라 권한을 승인
            return CameraPermission.authorize
            
        case .notDetermined: // 카메라 권한이 아직 결정되지 아직 않은 상태(초기 상태)
            let granted = await AVCaptureDevice.requestAccess(for: .video)
            return (granted) ? CameraPermission.authorize : CameraPermission.notDetermine

        case .denied: // 사용자가 권한을 거부한 상태
            return CameraPermission.deny
            
        case .restricted: // 접근 권한이 제한된 상태
            return CameraPermission.restrict
